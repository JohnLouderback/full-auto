...
(() => {
  if ((globalThis as Record<string, unknown>).childProcessHooksInstalled) {
    console.log("child process hooks already installed");
    return;
  }
  ...
  function isStringArray(arg: any): arg is readonly string[] {
    return (
      arg instanceof Array && arg.every((item) => typeof item === "string")
    );
  }
  ...
  (globalThis as Record<string, unknown>).childProcessHooksInstalled = true;
  console.log("child process hooks installed");
})();
